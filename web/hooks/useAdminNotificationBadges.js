...
/** Task 30 (+Task 41 for deletion requests) — numbered badges for Users (new
 * registrations), Reports (bug/support requests), Book Moderation (listing +
 * user reports) and Account Deletion Requests, clearing the moment the admin
 * opens that section. */
export function useAdminNotificationBadges(pathname) {
  const [counts, setCounts] = useState({ newUsers: 0, newReports: 0, newModeration: 0, newDeletionRequests: 0 });
  ...
  // Clear the badge for whichever of the 4 sections the admin just navigated into.
  useEffect(() => {
